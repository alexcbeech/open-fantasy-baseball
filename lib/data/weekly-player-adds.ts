 ...
 */
export async function getWeeklyPlayerAdds(client: PoolClient, leagueId: string, teamId: string) {
  const result = await client.query<{ add_limit: number; used: string | number }>(
    `select coalesce((l.settings->>'weeklyPlayerAddLimit')::integer, 6) as add_limit,
  ...
  return row ? { limit: Number(row.add_limit), used: Number(row.used) } : null;
}
