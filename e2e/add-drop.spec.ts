...
for (const action of ["add", "claim"] as const) {
  test(`full roster ${action} reviews both players before submitting`, async ({ page }) => {
    ...
    const review = detail.getByRole("button", { name: "Review Add and Drop" });
    ...
    await detail.getByRole("button", { name: "Change drop player" }).click();
    ...
    await detail.getByRole("button", { name: action === "add" ? "Confirm Add and Drop" : "Confirm Claim and Drop" }).click();
