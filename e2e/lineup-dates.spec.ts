...
test("past lineups lock moves; future moves send the chosen date", async ({ page }) => {
  ...
  const localToday = lineupToday(new Date(), "America/Los_Angeles");
  const tomorrow = shiftLineupDate(today, 1);
  await page.goto(`/team/team-1?date=${shiftLineupDate(today, -1)}`);
  await expect(page.getByText(shiftLineupDate(today, -1) === localToday ? "Today's lineup · locked" : "Past lineup · locked")).toBeVisible();
  ...
  await expect(page.getByLabel("Lineup date", { exact: true })).toHaveValue(localToday);
  if (localToday === today) {
