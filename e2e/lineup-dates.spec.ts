...
test("past lineups lock moves; future moves send the chosen date", async ({ page }) => {
  ...
  await page.getByLabel("Lineup date (ET)").fill(tomorrow);
  await page.getByRole("button", { name: "View day" }).click();
  await expect(page.getByLabel("Lineup date (ET)")).toHaveValue(tomorrow);
  ...
  await expect(page.getByLabel("Lineup date (ET)")).toHaveValue(today);
